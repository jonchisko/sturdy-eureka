...
fn setup(mut cmds: Commands, mut materials: ResMut<Assets<ColorMaterial>>) {
    ...
    cmds.insert_resource(Materials {
            ...
            food_material: materials.add(Color::rgb(1.0, 0.0, 0.8).into()),
            segment_material: materials.add(Color::rgb(0.25, 0.25, 0.25).into()),
    });
}

fn spawn_snake(mut cmds: Commands, materials: Res<Materials>, mut segments: ResMut<SnakeSegments>) {
    segments.0 = vec![
        cmds
...
fn position_translation(
    ...
    fn convert(pos: f32, bound_window: f32, bound_game: f32) -> f32 {
        let tile_px_size = bound_window / bound_game;
        pos * (bound_window / bound_game) - (bound_window / 2.) + (tile_px_size / 2.)
    }
    let window = windows.get_primary().expect("Could not get primary window");
